		    ...
		    <div class="large-12 columns">
				...
				<?php if($country) { ?>
					...
					?>
						    ...
						    <div class="row status-row" data-device="<?= $type_code ?>">
				...
				<?php } else { ?>
					...
					?>
							...
							<div class="row status-row" data-country="<?= $country_code ?>">
...
Checker.load = function() {
	...
	if ( Checker.mode == 'allCountriesOneDevice' ) {
		...
		Checker.countries.forEach( function( country ) {
				...
				Checker.loadStatus(	'byCountry', Checker.currentType, country )
		})
	}
	...
	if ( Checker.mode == 'oneCountryAllDevices' ) {
		...
		Checker.types.forEach( function( type ) {
				...
				Checker.loadStatus(	'byType', type, Checker.currentCountry );
		})
	}

}

Checker.loadStatus = function loadStatus(loadMode, type, country) {
	...
	$icon.removeClass().addClass('circlenumber fa fa-circle-o-notch fa-spin has-tip');

	if ( $icon ) {

		$.get("cached/nation="+country+"&type="+type+".temp")
		.done(function(data, status, xhr){
			...
			switch(data) {
				...
				case 'COMING_SOON':
					...
					$label.addClass("warning radius label").html('Coming soon')
				...
				case 'TEMPORARILY_OUT_OF_STOCK':
					...
					$label.addClass("alert radius label").html('Out of stock');
					
					//$label.removeAttr('title');
					//Foundation.libs.tooltip.getTip( $label ).remove();					
					//$label.attr("title", xhr.getResponseHeader('Last-Modified') )
					//console.log( xhr.getResponseHeader('Last-Modified') )
					//var moment = new moment( xhr.getResponseHeader('Last-Modified').toString() );
					//console.log( $label.find("span[data-selector=est-label]") )
					Foundation.libs.tooltip.getTip( $("#" + tip_id ) ).remove()
					$("#" + tip_id ).attr("title", 'Fetched ' + moment( xhr.getResponseHeader('Last-Modified') ).fromNow() )
					Foundation.libs.tooltip.create( $("#" + tip_id ) )
					//$("span[data-selector="+ tip_id +"]").html( 'Fetched ' + moment( xhr.getResponseHeader('Last-Modified') ).fromNow() + ' <span class="nub"></span>')
					//console.log( moment( xhr.getResponseHeader('Last-Modified') ).fromNow() )

				break;

				case 'UNAVAILABLE_COUNTRY':
					$icon.removeClass().addClass("circlenumber fa fa-remove");
					$label.addClass("alert radius label").html('Not available for this country')
				...
				default:
					...
					if ( data.indexOf('ERROR') || data.indexOf('Error') ) {
						...
						$label.addClass("nostatus radius label").html('Proxy down')
